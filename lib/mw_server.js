...
function ws_OnConnection(socket, ws) {
    ...
    socket.Emit = function(name, data) {
        ...
        if(socket.readyState === webSocket.OPEN) {
            ...
            console.log('Web Socket(' + socket.Id + ') send to ' + address +
                    '\n   ' + message);
            socket.send(message);
        }
    };
    ...
    socket.on('message', function(message) {
        ...
        if(message.substr(0, 1) === 'P') {
            ...
            if(idLen === stop) {
                console.log('MW WebSocket(' + socket.Id +
        ...
        if(socket.OnCalls[name] === undefined) {
            mw_fail('MW WebSocket(' + socket.Id +
                    ') on callback "' + name +
    ...
    socket.on('close', function() {
        ...
        console.log('MW WebSocket(' + socket.Id + ') closed from ' + address);
