...
var mw = {onCalls: {} }; // big catch all MW (mirror worlds) global object.
...
function ws_OnConnection(socket, ws) {
    ...
    // We are making an object like a socket.io socket connection
    // object but we are not overriding the on method that already
    // exists in "ws" WebSockets, by using "On" in place of "on".
    socket.On = function(name, func) {

        mw.onCalls[name] = func;
    };

    // Write to just this socket
    ...
    socket.Emit('initiate', 'Hello',  'Mirror Worlds');

    mw.Broadcast('subscriptions', [
            {name: 'name1', actorUrl: 'url1'},
            {name: 'name2', actorUrl: 'url2'}
    ]);
