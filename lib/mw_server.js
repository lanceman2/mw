...
function ws_OnConnection(socket, ws) {
    ...
    socket.Sources = []; // Sources that this client writes
    ...
    socket.On('subscribe', function(sourceId) {
        ...
        if(mw.sources[sourceId]) {
            ...
            console.log('MW subscribe ' + address + ' to ' +
                    mw.sources[sourceId].shortName);

            mw.sources[sourceId].sinks[socket.Id] = socket;
            socket.Sinks.push(sourceId);
            if(mw.sources[sourceId].payload) {
                socket.send(mw.sources[sourceId].payload);
                console.log('MW initialized subscription ' +
                        mw.sources[sourceId].jsSinkSrc + ' to ' + address +
    ...
    socket.on('close', function() {
        ...
        // remove this sockets subscriptions
        socket.Sinks.forEach(function(sourceId) {
        ...
        socket.Sources.forEach(function(sourceId) {

            var clients = mw.sources[sourceId].sinks;
            for(id in clients)
                clients[id].Emit('removeSubscription', sourceId);

            delete mw.sources[sourceId].sinks[socket.Id];
        });
        delete socket.Sources;
    });
