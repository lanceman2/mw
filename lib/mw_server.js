...
var path = require('path'),
    ...
    http = require('http').createServer(http_handler).
        listen(config.http_port, config.http_local),
    url = require('url'),
...
function ws_OnConnection(socket) {
    ...
    socket.on('close', function() {
        console.log('Web Socket ' + socket._socket.remoteAddress +
                ' closed');
    });
}
...
(new ws({server: https})).on('connection', ws_OnConnection);

// This server shares the same port as the http server
(new ws({server: http})).on('connection', ws_OnConnection);
...
// This just spews to stderr when the server is running, nothing more.
http.listen(() => {
    ...
    if(address.address.toString() === '::')
        ...
        console.error('Server listening at: http://' +
    ...
    if(address.address.toString() === '::')
        ...
        console.error('Secure server listening at: https://' +
                address.address + ':' + address.port + passcode) + '\n';
});
