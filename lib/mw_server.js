...
var
    ...
    wss = new webSocket({server: https}),
    ws = new webSocket({server: http});
...
function ws_OnConnection(socket, ws) {
    ...
    socket.send(JSON.stringify({ name: 'initiate',
        args: [ 'Welcome to Mirror Worlds' ]}));
    ...
    socket.on('message', function(message) {
        ...
        // echo like so
        //
        // try{ socket.send(message); } catch(e) { delete socket; }
    });
