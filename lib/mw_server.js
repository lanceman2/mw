...
var mw = {
    ...
    onCalls: {}, // for a list of socket.io like handlers
    ...
    sockets: {} // To get the client socket from the client Id
};
...
function ws_OnConnection(socket, ws) {
    ...
    socket.on('message', function(message) {
        ...
        if(message.substr(0, 1) === 'P') {
            ...
            mw.sources[sourceId].subscription.forEach(function(socket) {
    ...
    socket.Sources = []; // Sources that this client generates
    socket.Sinks = [];   // Sources this client reads
    mw.sockets[socket.Id] = socket;
    ...
    socket.On('createSource',
        function(cliendSourceId, shortName, description, jsSinkSrc) {
            ...
            socket.Sources.push(mw.sources[sourceId]);

            // Tell the source client the ID we assigned
            // and we may have changed the shortName and description.
    ...
    socket.on('close', function() {
        ...
        // remove this socket from source and sink lists:
        
        socket.Sources.forEach(function(source) {

            delete mw.sources[source.sourceId];
        });

        socket.Sinks.forEach(function(sink) {

            delete mw.sinks[sink.sourceId];
        });

        

        delete mw.sockets[socket.Id];
    });
